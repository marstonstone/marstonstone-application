...
function Navbar(props) {
  ...
  return (
    <Drawer variant="permanent" anchor="left" sx={navbarStyles.drawer} open={open}>
      <Box display="flex">
        <IconButton sx={{ mr: 1, color: 'white' }} onClick={handleDrawerOpen}>
          <MenuIcon />
        </IconButton>
        <Typography sx={navbarStyles.header} component="div">
          Marston Stone
        </Typography>
      </Box>
      ...
      <List>
        {/* <ListItemButton onClick={handleDrawerOpen}>
          {open && <MenuIcon />}
          <ListItemText>Marston Stone</ListItemText>
        </ListItemButton> */}
        <Divider />
        ...
        <ListItemButton onClick={() => navigate('Home')}>
          ...
          <ListItemText sx={navbarStyles.text}>Home</ListItemText>
        </ListItemButton>
          ...
          <Box key={id}>
            ...
            {children.map(({ id: childId, icon, active, route }) => (
              <ListItem disablePadding key={childId}>
                <ListItemButton selected={active} onClick={() => navigate(route)}>
